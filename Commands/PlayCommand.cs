...
namespace Climp.Commands
{
    public class PlayCommand : Command
    {
        ...
        public PlayCommand(Config config, MediaIndex mediaIndex)
            => (_config, _mediaIndex) = (config, mediaIndex);
        ...
        public override void Execute(Context context, State state, IReadOnlyList<string> arguments)
        {
            ...
            else
            {
                ...
                state.VlcProcess = Process.Start(_config.VlcExecutablePath.FullName, $"--intf dummy --vout dummy --dummy-quiet --no-repeat --play-and-exit \"{matchedMediaFile.File.FullName}\"");
